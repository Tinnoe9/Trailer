  ...
  return (
    <>
      <h1 className="text-2xl font-bold  mt-10 mb-5">
        <Link href="/Daily">{which}</Link>
      </h1>
      <div className="flex flex-wrap justify-around gap-5 ">
        {movies.slice(0, 6).map((movie) => (
          /*redirect to details page dynamic route*/
          <Link href={`/Details/${movie.id}`} key={movie.id}  className="w-50">
            <div >
              <div
                className="mb-2 l">
                  <Image src={`https://image.tmdb.org/t/p/w1280${movie.poster_path}`} alt={movie.title} width="350" height="300"/>
                </div>
